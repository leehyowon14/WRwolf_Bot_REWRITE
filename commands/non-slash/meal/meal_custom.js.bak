...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (args.length > 4 || args.length < 3) {
            return message.reply({ content : `잘못된 인자입니다.`, allowedMentions: {repliedUser: true} });
        }
        let date = args[0].split('.')
        if (date.length != 2) return message.reply({ content : `잘못된 인자입니다.\n사용 예시: !급식 3.1`, allowedMentions: {repliedUser: true} });
